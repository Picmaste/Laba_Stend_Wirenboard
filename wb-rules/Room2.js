defineVirtualDevice("Room2_L3",  {
  
  
  title: "Спальня RGB  подсветка  ",
 	cells: {
...
defineVirtualDevice("Room2_L4",  {
  
  title: "Спальня Димируемая подсветка ",
 	cells: {
...
defineVirtualDevice("Room2_L1",  {
  title: "Спальня основной свет",
 	cells: {
...
defineVirtualDevice("Room2_roll",  {
...
/////
	...
	var device_roll = "wb-mr6cu_34"; 
	var rele_up = "K1"
	var rele_dn = "K2"

    var device_input_button_up = "wb-gpio/EXT1_IN8";
	var device_input_button_dn = "wb-gpio/EXT1_IN9"
   ...
   // var delay_Timer=20;  // время хода штор 
	...
	var _Room_tag = "Room2_roll/tag";
	var _Room_stop ="Room2_roll/stop";
	var _Room_dn = "Room2_roll/dn";
	var _Room_up = "Room2_roll/up";

	
	var _Room = "Room2_roll";
	var _Room_delay = "Room2_roll/delay_Timer";
...
defineRule({  //  RGB подсветка Спальни
    whenChanged: ["Room2_L3/switch"], // правило сработает, когда значение параметра изменится на истинное 
    then: function (newValue, devName, cellName) {
      
    dev["wb-led_83/Channel 1"] = newValue ;
    dev["wb-led_83/Channel 2"] = newValue ;
    dev["wb-led_83/Channel 3"] = newValue ;
   }
});


defineRule({  //  RGB подсветка Спальни - Красная 
    whenChanged: ["Room2_L3/red"], // правило сработает, когда значение параметра изменится на истинное 
    then: function (newValue, devName, cellName) {
  log("Room2_L3/red  - {} ",  newValue);
    dev["wb-led_83/Channel 2 Brightness"] = newValue/2.55 ;
     }
});


defineRule({  //  RGB подсветка Спальни - Зелёная 
    whenChanged: ["Room2_L3/green"], // правило сработает, когда значение параметра изменится на истинное 
    then: function (newValue, devName, cellName) {
  log("Room2_L3/green  - {} ",  newValue);
    dev["wb-led_83/Channel 3 Brightness"] = newValue/2.55 ;
     }
});


defineRule({  //  RGB подсветка Спальни - Зелёная 
    whenChanged: ["Room2_L3/blue"], // правило сработает, когда значение параметра изменится на истинное 
    then: function (newValue, devName, cellName) {
  log("Room2_L3/blue  - {} ",  newValue);
    dev["wb-led_83/Channel 1 Brightness"] = newValue/2.55 ; 
     }
});


defineRule({  //  RGB подсветка Спальни
    whenChanged: ["Room2_L3/dim"], // правило сработает, когда значение параметра изменится на истинное 
    then: function (newValue, devName, cellName) {
  
    dev["wb-led_83/Channel 1 Brightness"] = newValue ;
    dev["wb-led_83/Channel 2 Brightness"] = newValue ;
    dev["wb-led_83/Channel 3 Brightness"] = newValue ;
...
defineRule({  //  Спальня Свет
    whenChanged: ["wb-gpio/EXT1_IN2","Room2_L1/switch"], // правило сработает, когда значение параметра изменится на истинное 
    then: function (newValue, devName, cellName) {
  
      log("newValue = {} , devName = {} , cellName = {} ",newValue, devName, cellName);
      
      
    if (devName=="Room2_L1"){
    dev["wb-mr6cu_32/K3"] = newValue ;
    return;
   }
     
  if (newValue) {
    dev["wb-mr6cu_32/K3"] = !dev["wb-mr6cu_32/K3"];
  	}	
  }
...
defineRule({  //    Спальня подсветка 
   whenChanged:  ["wb-gpio/EXT1_IN3","Room2_L2/switch"], // правило сработает, когда значение параметра изменится на истинное 
    then: function (newValue, devName, cellName) {
    ...
    return;
      ...
      if (newValue){
      dev["wb-mr6cu_32/K4"] = !dev["wb-mr6cu_32/K4"];
...
defineRule({  // Спальня  Подсветка 
   whenChanged:  ["wb-mdm3_23/Input 4","wb-mdm3_23/Input 3","Room2_L4/switch"], // правило сработает, когда значение параметра изменится на истинное 
  then: function (newValue, devName, cellName) {
 
    log("Log Спальня  Подсветка  1 - devName= {} ,cellName= {} =, Value=  {}  ",devName,cellName,newValue);  
  
    if (devName=="Room2_L4") {
    dev["wb-mdm3_23/K2"] = newValue ;
	 log("Log Спальня  Подсветка  2- devName= {} ,cellName= {} =, Value=  {}  ",devName,cellName,newValue);
      return;   
    }
   ...
   // setTimeout(function () {
	 ...
	 dev["Room2_L4/switch"]= dev["wb-mdm3_23/K2"];
     dev["Room2_L4/dim"]=dev ["wb-mdm3_23/Channel 2"]  ;
  //    log("Log Спальня  стоп switch= {}, dim= {} ",dev["wb-mdm3_23/K2"],dev ["wb-mdm3_23/Channel 2"]  );
 //   }, 1000);
   
   log("Log Спальня  старт  switch= {}, dim= {} ",dev["wb-mdm3_23/K2"],dev ["wb-mdm3_23/Channel 2"]  )
    } //if
...
defineRule({  // Спальня Яркость Подсветки
   whenChanged:  ["wb-mdm3_23/Channel 2"], // правило сработает, когда значение параметра изменится на истинное 
  then: function (newValue, devName, cellName) {
...
defineRule({  // Спальня Яркость Подсветки
   whenChanged:  ["Room2_L4/dim"], // правило сработает, когда значение параметра изменится на истинное 
  then: function (newValue, devName, cellName) {
   
    log("Log Спальня  Подсветка (Room2_L4/dim) - devName= {} ,cellName= {} =, Value=  {}  ",devName,cellName,newValue);
    
   dev["wb-mdm3_23/Channel 2"] = newValue ;
